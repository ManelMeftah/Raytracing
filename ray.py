...
def rendering( cam, contact, txt):
	if(txt == 'horizontale'):
		contact.color = textureHorizontale(contact.pt)
	elif (txt == 'damier'):
		contact.color = textureDamier(contact.pt)
	if 1.0 == contact.t :
	...
	for xpix in range( -cam.hsizewin, cam.hsizewin+1, 1):
		for zpix in range( -cam.hsizewin, cam.hsizewin+1, 1):
			...
			if None==contacts:
				...
				(r, v, b) = rendering( cam, contact1, txt) 
			img.putpixel( (xpix+cam.hsizewin, 2*cam.hsizewin-(zpix+cam.hsizewin)), (r,v,b))
	img.show()
	img.save( cam.nom + '.png')

#########################################
# ci dessous sont definies qqs primitives 
#########################################

def textureHorizontale(pt):
	(x, y, z) = pt
	z = int(z*10)
	#print("un passage par z = ", z, "zmod2 = ", z%2)
	if(0 == z%2):
		return (255, 255, 255)
	else:
		return (30, 30, 30)

def textureDamier(pt):
	(x, y, z) = pt
	x = int(x*10)
	y = int(y*10)
	z = int(z*10)
...
def essai_tore():
	...
	camera=Camera( oeil, droite, regard, vertical, 1.5, 100, normalize3((0., -1., 2.)))
	camera.nom="tore"
	raycasting( camera, Prim( tore( 0.4, 1), (255,200, 255)), 'none')
...
def essai_roman():
	...
	camera=Camera( oeil, droite, regard, vertical, 1.5, 100, normalize3((0., -1., 2.)))
	camera.nom="roman"
	raycasting( camera, Prim( roman(), (255,200, 255)), 'horizontale')
...
def essai_inter():
	...
	Inter( Prim( boule( (-0.5,0.,0.), 1), (255,0,0)),
	       Prim( boule( (0.5, 0.,0.), 1), (0,255,255))), 'none')
...
def essai_union():
        ...
        camera.nom="union"
        raycasting( camera, \
        Union( Prim( boule( (-.5,0.,0.), 1), (255,0,0)),
               Prim( boule( (.5, 0.,0.), 1.), (0,255,255))), 'none')
...
def essai_inside():
        ...
        camera=Camera( oeil, droite, regard, vertical, 1.5, 50, normalize3((0., -1., 2.)))
        camera.nom="inside"
        raycasting( camera, Prim( boule( (0., 0.,0.), 1), (0,255,255)), 'none')
